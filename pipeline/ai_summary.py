...
def generate_summaries(
    ...
    *,
    ...
    if generated:
        _write(summaries_path, updated, score_map, newly_written)
...
def _write(
    ...
    newly_written: set[str] | None = None,
) -> None:
    ...
    try:
        if path.exists():
            existing_dates = json.loads(path.read_text()).get("summary_dates") or {}
    except (json.JSONDecodeError, OSError):
        pass
    updated_dates = dict(existing_dates)
    for pid in (newly_written or summaries):
        updated_dates[pid] = today
    payload = {
        "generated_at": today,
        "model": _MODEL,
