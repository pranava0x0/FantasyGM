...
**Owner privacy:** never store the names of league owners/members. Track teams by stable ESPN `team.id` and `team.abbrev` so renames don't break continuity.

**Stack:** Python 3.9 + pydantic v2 + httpx for the pipeline; vanilla HTML/CSS/JS for the page (no framework, no build step). SQLite is overkill for ~50 KB/day of JSON; we just commit the daily snapshots.

**Operational scope (intentional non-goals):**

- No automated CI run of the pipeline. (Cookies in CI = too much blast radius.)
- No projection sources beyond ESPN until backlog item is promoted. Triangulation is on [BACKLOG.md](BACKLOG.md), not in scope yet.
...
- **Each pro game carries its tip-off time, and that's what makes locks knowable.** `settings.proTeams[*].proGamesByScoringPeriod[<period>][*]` has `date` (epoch **milliseconds**, UTC) plus `validForLocking` — present for *future* games, not just played ones. Scoring periods are 1-day windows, so this is a per-day schedule with times. `schedule.games_by_period()` normalizes it; `lineups.py` uses it to pin players whose game already tipped (ESPN locks per-player at tip-off, so a "move" for a locked player is one ESPN will refuse). `startTimeTBD: true` means no time yet — treat as *not* locked, i.e. still movable.
- **The in-progress matchup lives in `schedule`, not in any team object.** `league_raw.schedule[*]` where `matchupPeriodId == status.currentMatchupPeriod` gives `home`/`away` `{teamId, totalPoints}` with live running scores. `matchup_history` in state is *completed* weeks only (it deliberately skips 0–0 pairings); `current_matchup` is the in-flight one. Don't try to derive "who am I playing" from the history.
- **Mid-matchup scores look wrong next to season averages, and aren't.** A team averaging ~484 pts/week will show ~235 mid-week. Any "who's winning / win odds" math must model *remaining* games in the period — comparing a partial score to a full-week average silently compares different things. This is exactly why the spec'd win-odds estimate was pulled (docs/gm-console-spec.md §10.2).
- **The playoff bracket has no labels and no usable `winner` field.** `schedule[*].winner` is `null` on *every* game in this league, finished ones included, and no game carries a round name. `pipeline/season.py` infers both: rounds come from `scheduleSettings.matchupPeriodCount` (anything past it is playoffs) plus `teams[*].playoffSeed` (in the championship bracket only when `seed <= playoffTeamCount`), and winners come from the scores. **A bracket game at `0–0` has not been played** — ESPN pre-creates each pairing with zeroed totals the moment it's known, so `0–0` must read as *undecided*, never as a tie or a result. That one rule is what stops the UI crowning a champion mid-bracket; `tests/test_season.py::TestUndecidedBracket` pins it.
- **At season's end the waiver machinery correctly returns zeros.** `status.transactionScoringPeriod` advances past the last scheduled game (116 on 2026-08-25, while the final WNBA games are period 115). `upcoming_week_periods` starts there by design — it's the first period a new pickup actually plays — so every projection, net gain, and bid band goes to 0.0. **Do not "fix" this by clamping the window back to the last period with games:** an add made then genuinely cannot score, and back-dating the window would fabricate value. The right response is to explain the zeros in the UI (the season banner does), not to invent numbers.

- **WNBA `proTeamId` ≠ NBA convention.** Do not guess these from NBA habits — the IDs are independent. Discovered the hard way: `3` is Dallas (not Atlanta), `6` is LA Sparks (not Connecticut), `19` is Chicago (not Washington). The 2026 expansion teams have 6-digit IDs: `129689` = Golden State, `131935` = Toronto Tempo, `132052` = Portland Fire. Canonical map lives in `pipeline/build_state.py:WNBA_TEAM_ABBR`; refresh with `python scripts/refresh_pro_teams.py` (no auth needed — uses the public `site.api.espn.com/.../wnba/teams` endpoint).
...
- **Don't spend a subagent to find code you could grep.** A search subagent carries fixed overhead (its own system prompt, tool schemas, a verbose report) and returns a *summary* — for a "where is X?" question on a small/greppable codebase (these repos are ≈6–7k lines), a literal `grep`/`rg` for the mechanism + a targeted `Read` is cheaper *and* more complete: a concept-search agent can silently miss a call site a grep can't. Reserve subagents for real fan-out across many whole files or large unfamiliar trees. Don't double-search (grep *or* agent, not both). Decision ladder + scar tissue in [AGENTS.md](AGENTS.md) § "Search economics."
